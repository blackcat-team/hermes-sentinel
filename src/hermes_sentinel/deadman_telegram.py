"""Independent dead-man Telegram delivery (Stage H1B-1).

The sender-only delivery primitive for the accepted Stage H1A
``DeadManNotification`` intents, deliberately SEPARATE from the
central Sentinel's Stage E2 Telegram transport: the external
dead-man holds its own Stage-H bot credential and its own module,
imports nothing from ``hermes_sentinel.telegram`` and never touches
the central notification path. One notification intent is rendered
as one deterministic minimal plain-text message and produces exactly
ONE outbound Telegram Bot API ``sendMessage`` attempt.

Contract points (see docs/ARCHITECTURE.md section 32):

- the input is exactly one pending H1A ``DeadManNotification``
  (the future H1B runtime passes the OLDEST pending intent); the
  sender performs no queue selection, no ordering decision and —
  critically — no state acknowledgement: acknowledging belongs to
  the later orchestration, only after a successful delivery;
- the rendering is intentionally minimal for H1B (three plain lines:
  the fixed title, the kind line ``DOWN`` / ``RECOVERED``, and the
  confirmation moment). It uses ONLY facts present in the
  notification and invents no reachability or outage details; the
  later Stage I0 owns the Telegram UX redesign;
- ``DeadManTelegramSettings`` is immutable; the bot token is a
  secret validated against a conservative token alphabet, excluded
  from the dataclass repr and never surfaced in error text;
- the Telegram API origin is fixed (``https://api.telegram.org``)
  with no configurable API base URL; ``sendMessage`` is the only
  Telegram method invoked, and the dead-man bot is sender-only:
  this module never fetches inbound updates, never registers an
  inbound endpoint and never runs a receiving loop;
- one ``send()`` performs at most one HTTP request attempt: one
  ...
  a delivery failure;
- success requires HTTP 200 AND a bounded response body read that
  parses as JSON with top-level ``"ok"`` exactly ``true``;
- expected delivery failures raise ``DeadManTelegramDeliveryError``
  with concise messages that never contain the bot token, the
  authenticated request URL, any part of the response body or raw
  urllib exception text; unsafe exception chaining is suppressed.
  ...
  through one coherent mechanism, and a bounded delivery failure
  already determined for the exchange always wins over a concurrent
  cleanup failure;
- there are no retries, no backoff, no queue, no persistence and no
  clock: orchestration belongs to the later H1B runtime units.
"""
...
_MESSAGE_TITLE = "Hermes Sentinel Dead-Man"
...
class DeadManTelegramSettings:
    """Immutable dead-man Telegram delivery settings (Stage H1B-1).

    A SEPARATE Stage-H credential from the central Sentinel bot:
    nothing is shared with ``TelegramSettings`` beyond the same
    conservative validation rules. ``bot_token`` is a secret: it is
    excluded from the dataclass repr, validated against the
    conservative token alphabet (ASCII letters, digits, ``:``, ``_``
    and ``-``) so it can only ever occupy one safe URL path
    component, used verbatim (never stripped or normalized) and
    never surfaced in application error text. ``chat_id`` may be
    negative (Telegram groups/supergroups) but is never zero and
    never a bool. ``message_thread_id`` is ``None`` or a positive
    integer. No environment parsing or secret loading happens here;
    runtime configuration wiring belongs to the later H1B units.
    """
...
def render_deadman_message(notification: DeadManNotification) -> str:
    """Render the dead-man notification as its exact minimal text.

    The intentionally minimal three-line H1B shape (example)::

        Hermes Sentinel Dead-Man
        DOWN
        At: 2026-09-24T12:00:00+00:00

    Every fact is projected from the canonical notification: the
    fixed title, the kind line (``DOWN`` / ``RECOVERED``) and
    ``notification.at.isoformat()`` verbatim. There is deliberately
    no host line, no state line and no reachability detail — the
    dead-man watches the Sentinel failure domain itself, and the
    notification carries exactly what H1A persisted. No parse mode,
    no escaping framework, no emoji, no truncation, no wall-clock
    read and no I/O; the H1A notification stays the canonical source.
    """
    if notification.kind is DeadManNotificationKind.DOWN:
        kind_line = "DOWN"
    elif notification.kind is DeadManNotificationKind.RECOVERED:
        kind_line = "RECOVERED"
    else:
        # DeadManNotificationKind is closed at exactly two members;
        # valid notifications can never reach this branch.
        raise ValueError(
            f"unknown dead-man notification kind: {notification.kind!r}"
        )
    return "\n".join(
        (
            _MESSAGE_TITLE,
            kind_line,
            f"At: {notification.at.isoformat()}",
        )
    )


class DeadManTelegramDeliveryError(Exception):
    """A bounded dead-man Telegram delivery failure.

    Concise and secret-safe by construction: the message never
    contains the bot token, the authenticated request URL, any part
    of the response body or raw urllib exception text.
    """
...
def _close_bounded_response(
    resource: CloseableLike, failure: DeadManTelegramDeliveryError
) -> DeadManTelegramDeliveryError: ...


@overload
def _close_bounded_response(
    resource: CloseableLike, failure: DeadManTelegramDeliveryError | None
) -> DeadManTelegramDeliveryError | None: ...


def _close_bounded_response(
    resource: CloseableLike,
    failure: DeadManTelegramDeliveryError | None,
) -> DeadManTelegramDeliveryError | None:
    ...
    try:
        ...
        if failure is None:
            return DeadManTelegramDeliveryError(
...
class _StdlibHttpsOpener:
    ...
    reused across ``send()`` calls and adds no retry, backoff or
...
def _render_json_body(
    message: str, settings: DeadManTelegramSettings
) -> bytes:
...
def _classify_bounded_response(
    ...
    if status != 200:
        return DeadManTelegramDeliveryError(
            f"Dead-man Telegram delivery failed:"
            f" {_http_failure_reason(status)}"
        )
    if len(raw) > _RESPONSE_BODY_LIMIT_BYTES:
        return DeadManTelegramDeliveryError(
            "Dead-man Telegram delivery failed: response body too large"
        )
    try:
...
class DeadManTelegramSender:
    """Bounded sender-only dead-man Telegram notifier.

    One ``send(notification)`` call renders the minimal dead-man
    message, performs at most one outbound ``sendMessage`` attempt
    through the opener seam, and returns quietly only on the full
    success contract: HTTP 200 AND a bounded response body whose
    top-level ``"ok"`` is exactly ``true``. Every expected delivery
    failure raises ``DeadManTelegramDeliveryError``; there are no
    retries, no queue and no persistence.

    The sender NEVER acknowledges H1A state: acknowledgement of the
    delivered intent is the later orchestration's responsibility,
    performed only after this method returns quietly.
    """
    ...
    def __init__(
        self,
        settings: DeadManTelegramSettings,
    ...
    def send(self, notification: DeadManNotification) -> None:
        """Deliver exactly one attempt for the pending notification.

        The notification is one pending H1A intent (the future H1B
        runtime passes the OLDEST pending intent); the sender renders
        its minimal text, performs exactly one POST to the fixed
        ``sendMessage`` endpoint and fails closed with
        ``DeadManTelegramDeliveryError`` on any non-200 status, any
        ...
        No H1A state is read, mutated or acknowledged here.
        """
        ...
        try:
            ...
            bounded = _close_bounded_response(
                error,
                DeadManTelegramDeliveryError(
                    f"Dead-man Telegram delivery failed: {reason}"
                ),
            )
            raise bounded from None
        except TimeoutError:
            raise DeadManTelegramDeliveryError(
                "Dead-man Telegram delivery failed: timed out"
            ) from None
        except (OSError, http.client.HTTPException):
            ...
            raise DeadManTelegramDeliveryError(
                "Dead-man Telegram delivery failed: network error"
            ) from None
